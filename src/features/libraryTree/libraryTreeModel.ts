...
export function getAssetTagSuggestions(asset: Asset | null, assets: Asset[], folders: VirtualFolder[]) {
  ...
  const folderContextTagIds = getAssetFolderContextTagIds(asset, folders);
  ...
  const parentNameText = normalizeLibraryMatchText(getAssetImmediateParentName(asset.path));
  const searchText = normalizeLibraryMatchText([asset.name, asset.path, asset.extension, asset.type, parentNameText].join(" "));
  ...
  return libraryTagDefinitions
    ...
    .sort((first, second) => second.score - first.score || compareText(first.tagId, second.tagId))
    .slice(0, 16)
    .map(({ tagId }) => tagId);
}
...
function scoreAssetTagSuggestion(
  ...
  const termScore = getAssetTagSuggestionTermScore(tagDefinition, context.nameText, context.parentNameText, context.searchText);
...
function getAssetTagSuggestionTermScore(tagDefinition: LibraryTagDefinition, nameText: string, parentNameText: string, searchText: string) {
  let bestScore = 0;
  ...
  for (const term of libraryTagSuggestionTermsById.get(tagDefinition.id) ?? []) {
    ...
    if (nameText === term || parentNameText === term) {
    ...
    if (normalizedTextIncludesTerm(parentNameText, term)) {
      bestScore = Math.max(bestScore, term.includes(" ") ? 92 : 72);
      continue;
    }
...
function getAssetFolderContextTagIds(asset: Asset, folders: VirtualFolder[]) {
  const tags = new Set<string>();

  collectAssetFolderContextTagIds(asset, folders, tags);
  return tags;
}

function collectAssetFolderContextTagIds(asset: Asset, folders: VirtualFolder[], tags: Set<string>) {
  ...
  for (const folder of folders) {
    ...
    const matchedChildFolder = collectAssetFolderContextTagIds(asset, folder.children, tags);
...
function getAssetImmediateParentName(path: string) {
  ...
  return directoryPath ? getAssetBaseName(directoryPath) : "";
}

...
export function VectorImagePreview({ asset }: { asset: VectorImageAsset }) {
  ...
  const { errorMessage, imageUrl, isLoading } = useVectorImageObjectUrl(asset);
  const dragStartRef = useRef<{ offsetX: number; offsetY: number; pointerId: number; x: number; y: number } | null>(null);
  const message = errorMessage ?? decodeErrorMessage ?? (isLoading ? "Loading SVG..." : "");
  ...
  return (
    ...
    >
      {imageUrl && !decodeErrorMessage ? (
        <img
          alt={asset.name}
          className="absolute left-1/2 top-1/2 h-[82%] w-[82%] select-none object-contain"
          draggable={false}
          src={imageUrl}
          style={{
          ...
          onError={() => {
            setDecodeErrorMessage(`Could not render SVG: .${asset.extension} is not supported by the preview renderer yet.`);
          }}
        />
      ) : null}
      {message ? (
...
export function VectorImageThumbnail({ asset, fallback }: { asset: VectorImageAsset; fallback: ReactNode }) {
  const [decodeFailed, setDecodeFailed] = useState(false);
  const { errorMessage, imageUrl } = useVectorImageObjectUrl(asset);
...
export function isVectorImageExtension(extension: string) {
  return extension.toLowerCase() === "svg";
}

function useVectorImageObjectUrl(asset: VectorImageAsset) {
  ...
  useEffect(() => {
    ...
    async function loadImage() {
      try {
        ...
        const nextUrl = URL.createObjectURL(new Blob([toArrayBuffer(bytes)], { type: "image/svg+xml" }));
        ...
        if (!cancelled) {
          setErrorMessage(`Could not render SVG: ${String(error)}`);
          setIsLoading(false);
        }
      }
    }
